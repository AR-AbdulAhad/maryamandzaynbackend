...
function mapNews(n) {
  return {
    ...
    image: n.image,
    tagline: n.tagline,
    button_text: n.buttonText,
    button_link: n.buttonLink,
    active: n.active,
    order: n.order,
...
export async function addNews(req, res) {
  ...
  return res.status(201).json({ news: mapNews(news) });
}
...
export async function editNews(req, res) {
  ...
  return res.json({ news: mapNews(news) });
}
